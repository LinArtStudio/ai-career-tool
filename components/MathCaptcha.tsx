...
export default function MathCaptcha({ onVerify }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setCountdown((prev) => {
        if (prev <= 1) { clearInterval(timer); setCanSubmit(true); return 0; }
        return prev - 1;
      });
    }, 1000);
  ...
  const generate = () => {
    ...
    if (newOp === "×") { newA = Math.floor(Math.random() * 12) + 2; newB = Math.floor(Math.random() * 12) + 2; }
    else if (newOp === "-") { newA = Math.floor(Math.random() * 50) + 10; newB = Math.floor(Math.random() * newA); }
    else { newA = Math.floor(Math.random() * 50) + 10; newB = Math.floor(Math.random() * 50) + 10; }
    setA(newA); setB(newB); setOp(newOp);
    setAnswer(""); setError(""); startTime.current = Date.now();
  };

  const getCorrect = () => { if (op === "+") return a + b; if (op === "-") return a - b; return a * b; };

  const check = () => {
    if (honeypotRef.current && honeypotRef.current.value) { setError("验证失败"); generate(); return; }
    const elapsed = Date.now() - startTime.current;
    if (elapsed < 2000) { setError("请认真计算后再提交"); generate(); return; }
    if (attempts >= 5) { setError("验证失败次数过多，请刷新页面重试"); return; }
  ...
  return (
    <div className="p-4 bg-gray-50 border rounded-lg">
      ...
      <div className="flex items-center gap-3">
        <span className="text-xl font-mono font-bold bg-white px-3 py-2 rounded border">{a} {op} {b} = ?</span>
        <input type="number" value={answer} onChange={(e) => setAnswer(e.target.value)}
          onKeyDown={(e) => e.key === "Enter" && canSubmit && answer && check()}
          className="w-20 border rounded px-3 py-2 text-center text-lg" placeholder="?" disabled={!canSubmit} />
        <button onClick={check} disabled={!canSubmit || !answer}
